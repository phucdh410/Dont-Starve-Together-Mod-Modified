...
local function IsPrefab(item, inst)
	...
	local brainseends = inst.components.container:FindItem(function(v)
    ...
    elseif item.components.crop ~= nil then
        ...
        if item.components.pickable ~= nil then
            return item.components.pickable.canbepicked
                    ...
                    or item.components.pickable.product == "cutreeds"
					or item.components.pickable.product == "cutlichen"
                    or item.components.pickable.product == "red_cap"
                    or item.components.pickable.product == "blue_cap"
					or item.components.pickable.product == "green_cap"
					or item.components.pickable.product == "wormlight_lesser"
					or item:HasTag("plantedsoil") and item:HasTag("farm_plant")
					or item.components.pickable.product == "compost"
					or item.components.pickable.product == "cave_banana"
					or item.components.pickable.product == "peach"
					or item.components.pickable.product == "petals_lily"
					or item.components.pickable.product == "petals_rose"
					or item.components.pickable.product == "petals_orchid"
					or item.components.pickable.product == "squamousfruit"
					or item.components.pickable.product == "shyerry"
					)
        end
    end
end
...
local function hasTree(item, inst)
    ...
    return item.prefab == "evergreen"
			...
			or item.prefab == "moon_tree"
            or item.prefab == "twiggytree"
			or item.prefab == "shyerrytree1"
			or item.prefab == "shyerrytree2"
			or item.prefab == "shyerrytree3"
			or item.prefab == "shyerrytree4"
            or item.prefab == "palmcone"
            or item.prefab == "palmconetree"
...
local function PickBerriesAction(inst)
    ...
    if target then
        ...
        elseif target:HasTag("mushroom_farm") and target.components.harvestable.produce >= 4 then
            return BufferedAction(inst, target, ACTIONS.HARVEST)			
        elseif  target.components.pickable and target.components.pickable.canbepicked and not target.components.pickable.caninteractwith == false 
		        then
...
local function chopTree(inst)
    ...
    if target then
        if not target:HasTag("stump") then
            if (target.components.growable and target.components.growable.stage == 3 ) 			
			or target.prefab == "shyerrytree1"
			or target.prefab == "shyerrytree2"
			or target.prefab == "shyerrytree3"
			or target.prefab == "shyerrytree4"
			or target.components.growable  == nil
...
local function makeNewSmallPlantAction(inst)
	...
	local smallseends = inst.components.container:FindItem(function(v)
        return     v.prefab == "dug_sapling"
		        or v.prefab == "dug_sapling_moon"
				or v.prefab == "dug_marsh_bush"               
                or v.prefab == "dug_grass" 
				or v.prefab == "cutted_orchidbush"              
                or v.prefab == "dug_orchidbush"  								
    end)
	...
	if smallseends then
        ...
        if inst.posPlant ~= nil then
            return BufferedAction(inst, nil, ACTIONS.AUTO_FARM_PLANT, smallseends , Vector3(inst.posPlant.x,inst.posPlant.y,inst.posPlant.z))
        end
    end
...
local function hateSkeleton(item, inst)
	...
	if item.prefab ~= nil 
        ...
        and item.components.workable:CanBeWorked()
		and item.prefab ~= "trophyscale_oversizedveggies"
	then
        local result = string.find(item.prefab, "_oversized")
        if result ~= nil then
            return true
        end
	elseif	item.prefab == "houndbone"
	then 	return true
...
local function hateNettable(item, inst)
    ...
    if item.prefab ~= nil and item.prefab ~= "raindonate" and item.prefab ~= "bee" and item.prefab ~= "mosquito" 
	    and item.components.workable ~= nil
        and item.components.workable.action == ACTIONS.NET
        and item.components.workable:CanBeWorked()
	then
        return true
    end		
...
local function isNettable(inst)
	...
	local hasnet = inst.components.container:FindItem(function(v)
