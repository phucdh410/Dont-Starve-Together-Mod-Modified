...
function Map:CanDeployBridgeAtPointWithFilterNotCave(pt, inst, mouseover, tilefilterfn)
    if tilefilterfn then
        local tile = self:GetTileAtPoint(pt.x, pt.y, pt.z)
        if not tilefilterfn(self, tile) then
            return false
...
function Map:CanDeployRopeBridgeAtPointNew(pt, inst, mouseover)
    if not TheWorld:HasTag("cave") then 
        return self:CanDeployBridgeAtPointWithFilterNotCave(pt, inst, mouseover, self.BridgeFilter_Void)
    end
...
function Map:CanDeployVineBridgeAtPoint(pt, inst, mouseover)
    if not TheWorld:HasTag("cave") then 
        return self:CanDeployBridgeAtPointWithFilterNotCave(pt, inst, mouseover, self.BridgeFilter_OceanAndVoid)
    end
