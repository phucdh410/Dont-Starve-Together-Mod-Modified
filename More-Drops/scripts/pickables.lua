...
pickables.ImproveGrass = function(inst)
    ...
    if utils.LootRandom(grassChance) then
		inst.components.pickable.numtoharvest = GetModConfigData("grassamount", KnownModIndex:GetModActualName("More Drops - DHPModified")) + 1
	else
		inst.components.pickable.numtoharvest = 1
	end
  if GetModConfigData("infinitefertilize", KnownModIndex:GetModActualName("More Drops - DHPModified")) == true then
...
pickables.ImproveSapling = function(inst)
	...
	if utils.LootRandom(twigChance) then
		inst.components.pickable.numtoharvest = GetModConfigData("twigamount", KnownModIndex:GetModActualName("More Drops - DHPModified")) + 1
	else	
		inst.components.pickable.numtoharvest = 1
	end
  if GetModConfigData("infinitefertilize", KnownModIndex:GetModActualName("More Drops - DHPModified")) == true then
...
pickables.ImproveReeds = function(inst)
	...
	if utils.LootRandom(reedChance) then
		inst.components.pickable.numtoharvest = GetModConfigData("reedamount", KnownModIndex:GetModActualName("More Drops - DHPModified")) + 1
	else
		inst.components.pickable.numtoharvest = 1
	end
	if GetModConfigData("infinitefertilize", KnownModIndex:GetModActualName("More Drops - DHPModified")) == true then
...
pickables.ImproveBerries = function(inst)
	...
	if utils.LootRandom(berryChance) then
		inst.components.pickable.numtoharvest = GetModConfigData("berryAmount", KnownModIndex:GetModActualName("More Drops - DHPModified")) + 1
	else 
		inst.components.pickable.numtoharvest = 1
	end
...
pickables.ImproveJuicy = function(inst)
	...
	if utils.LootRandom(berry_JuicyChance) then
		inst.components.pickable.numtoharvest = GetModConfigData("berry_JuicyAmount", KnownModIndex:GetModActualName("More Drops - DHPModified")) + 3
	else
