...
local logChance = GetModConfigData("logChance", KnownModIndex:GetModActualName("More Drops - DHPModified"))

local function ImproveTree(inst)
	--Do these when the tree is fully chopped
  local seed_chance  = GetModConfigData("treeseedchance", KnownModIndex:GetModActualName("More Drops - DHPModified"))
    ...
    if utils.LootRandom(logChance) then
		utils.DoTimes(GetModConfigData("extralogs", KnownModIndex:GetModActualName("More Drops - DHPModified")), inst.components.lootdropper.SpawnLootPrefab, inst.components.lootdropper, "log")
	end	
    ...
    elseif inst.name == "Twiggy Tree" and inst.components.growable.stage ~= 1 then
      if utils.DropLootRandom(inst, "twiggy_nut", seed_chance) then print("Dropped seed") end
		...
		if inst.components.growable.stage == 3 and utils.LootRandom(logChance) then
			utils.DoTimes(GetModConfigData("extralogstall", KnownModIndex:GetModActualName("More Drops - DHPModified")), inst.components.lootdropper.SpawnLootPrefab, inst.components.lootdropper, "log")
		end
