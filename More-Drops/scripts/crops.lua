...
local function ImproveCrop(component, inst)
	...
	component.Harvest = function(self, harvester)
		...
		if harvester ~= nil then
			if utils.LootRandom(GetModConfigData("cropChance", KnownModIndex:GetModActualName("More Drops - DHPModified"))) then
				for i = 1, GetModConfigData("extracrops", KnownModIndex:GetModActualName("More Drops - DHPModified")) do
