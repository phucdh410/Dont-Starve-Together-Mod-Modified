...
local function get_other_player_linked_bell(inst, other)
    if other.components.inventory ~= nil then
        return other.components.inventory:FindItem(function(item)
            return (item ~= inst) and (item.prefab == "koalebeef_summer_bell") and item:_HasBeefalo()
        end)
...
local function on_used_on_beefalo(inst, target, user)
    if target.SetBeefBellOwner ~= nil then
        ...
        if user and get_other_player_linked_bell(inst, user) ~= nil then
...
local function fn()
    ...
    inst.components.useabletargeteditem:SetTargetPrefab("koalebeef_summer")
...
return Prefab("koalebeef_summer_bell", fn, assets)
