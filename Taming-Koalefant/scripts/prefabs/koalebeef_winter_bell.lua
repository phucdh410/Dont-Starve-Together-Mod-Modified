...
local function get_other_player_linked_bell(inst, other)
    if other.components.inventory ~= nil then
        return other.components.inventory:FindItem(function(item)
            return (item ~= inst) and (item.prefab == "koalebeef_winter_bell") and item:_HasBeefalo()
        end)
...
local function fn()
    ...
    inst.components.useabletargeteditem:SetTargetPrefab("koalebeef_winter")
...
return Prefab("koalebeef_winter_bell", fn, assets)
