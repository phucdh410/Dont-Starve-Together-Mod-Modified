        ...
        function self.GetSkinOptions(s, ...)
            ...
            if ret then
                ...
                for k, v in pairs(s.skins_list) do
                    if
                        ret[k + 1] and ret[k + 1].image and v and v.item and itemskins[v.item] and
                            (itemskins[v.item].atlas or itemskins[v.item].image)
                     then
...
if thank_help then
    ...
    for k, v in pairs(tohook) do
        ...
        ThankYouPopup[k] = function(self, ...)
            ...
            local item =
                self.current_item and self.items and self.items[self.current_item] and
                self.items[self.current_item].item
            if item then
                local skin = GetSkin(item)
                if skin and skin.swap_icon then
                    self.spawn_portal:GetAnimState():OverrideSkinSymbol(
                        "SWAP_ICON",
                        skin.swap_icon.build,
                        skin.swap_icon.image
                    )
                elseif skin and skin.image and skin.atlas then
                    self.spawn_portal:GetAnimState():OverrideSkinSymbol(
                        "SWAP_ICON",
                        softresolvefilepath(skin.atlas),
                        skin.image .. ".tex"
                    )
                end
            end
