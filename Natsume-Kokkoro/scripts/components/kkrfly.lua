local kkrfly = Class(function(self, inst)
    self.inst = inst
	self.kkr_fly = false 	--是否正在飞行
...
function kkrfly:Fly()
	...
	if self.inst then
		...
		self.inst:ListenForEvent("equip", onequip) --穿装备解除飞行
		self.inst:ListenForEvent("unequip", onunequip)	--脱装备解除飞行
	end
