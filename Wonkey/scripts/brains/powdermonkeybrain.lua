...
local function shouldsteal(inst)
    ...
    if #ents > 0 then
        ...
        local bc = inst.components.crewmember and inst.components.crewmember.boat and inst.components.crewmember.boat.components.boatcrew or nil
        if bc and not inst.components.follower.leader then
            local target = nil
            local x,y,z = inst.Transform:GetWorldPosition()
        ...
        if not inst.components.combat.target then
            local target = FindEntity(
                    ...
                    function(guy)
                        ...
                        if not guy.components.inventory or guy.components.inventory:NumItems() == 0 or inst.components.follower.leader then
                            return false
                        end
...
local function GoToHut(inst)
    ...
    if home == nil
            ...
            or home:HasTag("burnt") or inst.components.follower.leader then
        return nil
    end
...
local function bananahandoff(inst)
    ...
    local queendist = inst:GetDistanceSqToInst(TheWorld.components.piratespawner.queen)
    if queendist > 60*60 then
        return nil
    end
...
function PowderMonkeyBrain:OnStart()
    ...
    local root = PriorityNode(
    {
        ...
        -- otherwise , tinker with stuff and
        ...
        ChattyNode(self.inst, "MONKEY_TALK_RETREAT",
            WhileNode(function() return TheWorld.state.iscavenight end, "Cave nightness",
