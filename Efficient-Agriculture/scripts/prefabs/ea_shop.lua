...
local function ChangeGoods(inst)
    for i = 1, 3, 1 do      --随机生成货物
        ...
        if prob <= 5 then
            ...
            inst.goods[i] = deepcopy(list_blueprint[probb])    --特殊蓝图 5%
            inst.goods[i].num = 1
        elseif prob <= 10 then
            probb = math.floor(math.random(1,74))
            inst.goods[i] = deepcopy(list_precious[probb])     --珍贵物品 5%
            inst.goods[i].num = 1
        elseif prob <= 40 then
            probb = math.floor(math.random(1,84))
            inst.goods[i] = deepcopy(list_resource[probb])     --基础资源 30%
            inst.goods[i].num = math.floor(math.random(3,10))
        elseif prob <= 70 then
            probb = math.floor(math.random(1,55))
            inst.goods[i] = deepcopy(list_cloth[probb])    --衣服 30%
            inst.goods[i].num = math.floor(math.random(1,2))
        else
            probb = math.floor(math.random(1,71))
            inst.goods[i] = deepcopy(list_smithing[probb])     --打造物品 30%
...
local function ongetitem(inst,giver,item)
    ...
    inst:DoTaskInTime(FRAMES, function ()   --给物品
        ...
        if prod.components.inventoryitem and giver and giver.components.inventory then     --如果物品可以放入物品栏
