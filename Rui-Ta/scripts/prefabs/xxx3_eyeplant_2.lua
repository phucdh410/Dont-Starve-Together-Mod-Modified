local assets =
{
    Asset("ANIM", "anim/xxx3_eyeplant_2.zip"),
...
local function shouldKeepTarget(inst, target)
    if target and target:IsValid() and target.components.health and not target.components.health:IsDead() then
        ...
        return distsq < TUNING.EYEPLANT_STOPATTACK_DIST*TUNING.EYEPLANT_STOPATTACK_DIST*4
    else
        return false
    end
end
...
local function fn()
	...
	inst.Light:SetColour(197/255,197/255,170/255)
    ...
    inst.AnimState:SetBank("xxx3_eyeplant_2")
    inst.AnimState:SetBuild("xxx3_eyeplant_2")
    ...
    inst.components.combat:SetRange(TUNING.EYEPLANT_ATTACK_DIST * 2)
    ...
    inst:SetStateGraph("SGxxx3_eyeplant_2")
...
return Prefab("xxx3_eyeplant_2", fn, assets)