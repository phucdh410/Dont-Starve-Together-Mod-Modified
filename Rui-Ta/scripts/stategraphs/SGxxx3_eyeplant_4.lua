    ...
    State{
        ...
        timeline=
        {
            TimeEvent(11*FRAMES, function(inst)
                local spike = SpawnPrefab("xxx3_eyeplant_4_icespike")
                spike.caster = inst
                local tar = inst.components.combat.target
                if tar and tar:IsValid() then
                    spike.Transform:SetPosition(tar.Transform:GetWorldPosition())
                end
...
return StateGraph("SGxxx3_eyeplant_4", states, events, "idle", actionhandlers)