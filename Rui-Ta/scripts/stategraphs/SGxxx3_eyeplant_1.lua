    ...
    State{
        ...
        onupdate = function(inst)
            if inst.components.combat.target and distsq(inst.components.combat.target:GetPosition(),inst:GetPosition())
                <= inst.components.combat:CalcAttackRangeSq(inst.components.combat.target) and inst.cloudincd == false then
    ...
    State{
        ...
        timeline=
        {
            TimeEvent(27*FRAMES, function(inst)
                if inst.cloudincd == false then
                    inst.cloudincd = true
                    inst.cloud = SpawnPrefab("xxx3_eyeplant_1_cloud")
                    inst.cloud.caster = inst
                    inst.cloud.Transform:SetPosition(inst.Transform:GetWorldPosition())
        ...
        events=
        {
            EventHandler("animqueueover", function(inst)
                if inst.components.combat.target and
                    ...
                    inst.components.combat:CalcAttackRangeSq(inst.components.combat.target)
                    and inst.cloudincd == false then
                    inst.sg:GoToState("attack")
                else
                    inst.sg:GoToState("idle")
                end
    ...
    State{
        ...
        timeline={
            TimeEvent(12 * FRAMES, function(inst)
                if inst.cloud and inst.cloud:IsValid() then
                    inst.cloud:FinishImmediately()
                end
            end),
        },
    },
...
return StateGraph("SGxxx3_eyeplant_1", states, events, "idle", actionhandlers)